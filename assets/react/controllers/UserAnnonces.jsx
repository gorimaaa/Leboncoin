...
const UserAnnonces = ({ userAnnonces }) => {
  ...
  const annoncesFiltrees = useMemo(() => {
    let resultats = userAnnonces.filter(annonce => 
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <header className="fixed top-0 w-full bg-gray-200 text-gray-900 shadow z-10">
        ...
        <Link to="/react" className="text-4xl font-semibold text-orange-600 hover:text-orange-700">
          LeBonCoin
        </Link>
          ...
          <div className="flex items-center space-x-4">
            ...
            <div className="ml-auto flex items-center space-x-4">
              ...
              <div className="flex flex-col items-center">
                <a href="/profil" className="text-gray-700 hover:text-gray-900">
      ...
      {annoncesFiltrees.map((annonce) => (
        <div key={annonce.id} className="group relative bg-white p-4 rounded-lg shadow space-y-2">
          ...
          <a href={`/modify/${annonce.id}`} className="text-indigo-600 hover:text-indigo-900 text-sm font-semibold">
            Modifier
          </a>
          {/* Ajout du bouton "Supprimer" */}
          <a href={`/delete/${annonce.id}`} className="text-red-600 hover:text-red-900 text-sm font-semibold ml-2">
                    Supprimer
                  </a>
        </div>
            ))}
